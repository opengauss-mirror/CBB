#!/usr/bin/env bash
# =============================================================================
# setup-pre-commit.sh -- install CBB pre-commit checks in an isolated venv.
# =============================================================================
set -euo pipefail

PIP_MIRROR="${PIP_MIRROR:-https://mirrors.aliyun.com/pypi/simple/}"
PIP_HOST="$(printf '%s' "$PIP_MIRROR" | sed -E 's#^https?://([^/]+)/?.*#\1#')"
PRE_COMMIT_VERSION="${PRE_COMMIT_VERSION:-2.21.0}"
...
git rev-parse --show-toplevel >/dev/null 2>&1 || die "run this script inside the CBB git repository"
...
log "pre-commit version: $PRE_COMMIT_VERSION"
