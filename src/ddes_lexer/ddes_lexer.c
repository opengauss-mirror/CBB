...
static bool32 lex_diag_num_word(word_t *word, text_t *text, num_part_t *np)
{
...
static status_t lex_skip_escape(lex_t *lex)
{
    char next = LEX_NEXT;
    switch (next) {
        case LEX_END:
            LEX_THROW_ERROR_EX(LEX_LOC, ERR_LEX_SYNTAX_ERROR, "text not completed");
            return CM_ERROR;
        case '\\':
        case 'r':
        case 'n':
...
static status_t lex_fetch_c_str(lex_t *lex, word_t *word)
{
    bool32 finished = CM_FALSE;
    char curr;
    ...
    while (curr != LEX_END) {
        if (curr == escchar) {
            if (lex_skip_escape(lex) != CM_SUCCESS) {
                LEX_THROW_ERROR_EX(LEX_LOC, ERR_LEX_SYNTAX_ERROR, "invalid escape character");
                return CM_ERROR;
            }
            continue;
        }
        if (curr == DOUBLE_QUOTATION) {
            (void)lex_skip(lex, 1);
            finished = CM_TRUE;
            break;
        }
...
static status_t lex_fetch_dquota(lex_t *lex, word_t *word)
{
    ...
    if (lex->flags & LEX_C_STRING) {
        return lex_fetch_c_str(lex, word);
    }
    ...
    if (word->text.len > CM_MAX_NAME_LEN) {
        LEX_THROW_ERROR_EX(word->text.loc, ERR_LEX_SYNTAX_ERROR, "text is too long, max is %u",
            CM_MAX_NAME_LEN);
        return CM_ERROR;
    }
...
status_t lex_expected_fetch_size(lex_t *lex, int64 *size, int64 min_size, int64 max_size)
{
    ...
    if (min_size != CM_INVALID_INT64 && max_size != CM_INVALID_INT64) {
    ...
    if (min_size != CM_INVALID_INT64 && *size < min_size) {
    ...
    if (max_size != CM_INVALID_INT64 && *size > max_size) {
...
static inline status_t lex_word2hexchar(word_t *word, char *c)
{
    ...
    do {
        ...
        val = cm_hex2int8((uchar)word->text.txt.str[2]);
        val <<= 4;
        val += cm_hex2int8((uchar)word->text.txt.str[3]);
        if (val > 127) {
            break;
        }
        *c = (char)val;
...
static inline status_t lex_str2hexchar(const char *str, char *c)
{
    ...
    do {
        ...
        val = cm_hex2int8((uchar)str[2]);
        val <<= 4;
        val += cm_hex2int8((uchar)str[3]);
        if (val > 127) {
            break;
        }
        *c = (char)val;
...
status_t lex_expected_fetch_asciichar(lex_t *lex, char *c, bool32 allow_empty_char)
{
    ...
    rc_memzero = (int)memset_sp(&word, sizeof(word_t), 0, sizeof(word_t));
    if (rc_memzero != EOK) {
        return CM_ERROR;
    }
    ...
    do {
        ...
        // escaped char = '
        cond = (word.text.len == 2) && CM_TEXT_BEGIN(&word.text.txt) == '\'' && CM_TEXT_SECOND(&word.text.txt) == '\'';
        if (cond) {
            *c = '\'';
            return CM_SUCCESS;
        }
        ...
        cond = CM_TEXT_BEGIN(&word.text.txt) == '\\' && CM_TEXT_SECOND(&word.text.txt) == 'x';
        if (cond) {
...
status_t lex_expected_fetch_str(lex_t *lex, char *str, uint32 str_max_length, char *key_word_info)
{
    ...
    do {
        ...
        if (word.text.len > str_max_length) {
            LEX_THROW_ERROR_EX(word.loc, ERR_LEX_SYNTAX_ERROR, "%s is too long, max length is %u", key_word_info,
                str_max_length);
            return CM_ERROR;
        }
        ...
        for (i = 0; i < word.text.len; i++) {
            ...
            if (word.text.str[i] == '\\' && i + 3 < word.text.len && word.text.str[i + 1] == 'x') {
                ...
                if (lex_str2hexchar(hex_str, &ret_c) == CM_SUCCESS) {
                    str[j] = ret_c;
                    j++;
