...
{
    if (np->digit_text.len > CM_MAX_INT32_PREC || np->has_dot || np->has_expn) {
        return NERR_ERROR;
    }

    if (np->digit_text.len == CM_MAX_INT32_PREC) {
        int32 cmp_ret = cm_compare_digitext(&np->digit_text, np->is_neg ? &g_neg_int32_ceil : &g_pos_int32_ceil);
        if (cmp_ret > 0) {
            return NERR_OVERFLOW;
        } else if (cmp_ret == 0) {
...
{
    if (np->digit_text.len > CM_MAX_UINT32_PREC || np->has_dot || np->has_expn || np->is_neg) {
        return NERR_ERROR;
    }
...
{
    if (np->digit_text.len > CM_MAX_UINT64_PREC || np->has_dot || np->is_neg || np->has_expn) {
        return NERR_ERROR;
    }
...
static inline num_errno_t cm_num_fetch_sign(num_part_t *np, int32 *i, const text_t *text)
{
    ...
    int32 precision)
{
    ...
    if (np->digit_text.len > num_text->len || np->digit_text.len >= CM_MAX_NUM_PART_BUFF) {
        CM_THROW_ERROR_EX(ERR_ASSERT_ERROR,
            "np->digit_text.len(%u) <= num_text->len(%u) and np->digit_text.len(%u) < CM_MAX_NUM_PART_BUFF(%d)",
...
status_t cm_text2uint32(const text_t *text_src, uint32 *value)
{
    ...
    if (text.len > CM_MAX_NUMBER_LEN) {
        CM_THROW_ERROR_EX(ERR_VALUE_ERROR, "Convert uint32 failed, the length of text can't be larger than %u",
            CM_MAX_NUMBER_LEN);
        return CM_ERROR;
    }
    CM_RETURN_IFERR(cm_text2str(&text, buf, CM_MAX_NUMBER_LEN + 1));
...
num_errno_t cm_numpart2bigint(const num_part_t *np, int64 *i64)
{
    ...
    if (np->digit_text.len == CM_MAX_INT64_PREC) {
        int32 cmp_ret = cm_compare_digitext(&np->digit_text, np->is_neg ? &g_neg_bigint_ceil : &g_pos_bigint_ceil);
        if (cmp_ret > 0) {
            return NERR_OVERFLOW;
        } else if (cmp_ret == 0) {
