...
| 通信类型 | 平均RTT | 吞吐量 | 适用场景 |
|---------|---------|--------|---------|
| IPC | ~150μs | ~6600 req/s | 同节点进程间通信 |
| TCP | ~200μs | ~5000 req/s | 跨节点网络通信 |

> 注：IPC模式经过优化后，性能比TCP快约25%，适合同节点高性能通信场景。
...
| --size | -s | 消息大小（字节），范围：64-65536<br>注意：小于64字节会自动调整 | 64 | `-s 1024` |
