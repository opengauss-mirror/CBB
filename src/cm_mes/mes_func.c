...
static status_t mes_check_task_threadpool_attr(mes_profile_t *profile)
{
    ...
    if (work_task_count_all_zero && !profile->tpool_attr.enable_threadpool) {
        LOG_RUN_INF("[mes][MES TASK THREADPOOL] work_task_count is zero and work threadpool is off");
        return CM_SUCCESS;
    }

    if (!work_task_count_all_zero) {
        if (!profile->tpool_attr.enable_threadpool) {
            LOG_RUN_INF("[mes][MES TASK THREADPOOL] work threadpool is off");
        } else {
            LOG_RUN_WAR("[mes][MES TASK THREADPOOL] work_task_count is not zero and work threadpool is on "
                "which is not allowed. so we turn off threadpool, don't worry, this is not a error");
...
int mes_connect(inst_type inst_id)
{
    ...
    if (ret != CM_SUCCESS) {
        cm_thread_unlock(&conn->lock);
        LOG_RUN_ERR("[mes]: MES_CONNECT failed, inst_id:%u.", inst_id);
        return ret;
    }
