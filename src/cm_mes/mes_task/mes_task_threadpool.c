...
bool8 mes_task_threadpool_start_thread()
{
    ...
    for (int i = 0; i < group_num; i++) {
        ...
        if (!cur_group->attr.enabled || !cur_group->inited) {
            continue;
        }
        ...
        LOG_DEBUG_INF("[MES TASK THREADPOOL][init] group start workers finished, group_id:%d",
...
void mes_task_threadpool_stop_thread()
{
    ...
    for (int i = 0; i < group_num; i++) {
        ...
        mes_task_threadpool_worker_t *worker = (mes_task_threadpool_worker_t*)cur_group->worker_list.head;
...
status_t mes_task_threadpool_init(mes_task_threadpool_attr_t *tpool_attr)
{
    ...
    mes_task_threadpool_start_thread();
...
void mes_put_msgitem_to_threadpool(mes_msgitem_t *msgitem)
{
    ...
    cm_atomic_inc(&group->history_put_cnt);
