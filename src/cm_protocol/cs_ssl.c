    ...
    char *tls13_cipher, uint32_t *tls13_offset)
{
    ...
    while (left.len > 0) {
        ...
        if (!support) {
            // match TLS1.3-cipher
            if (cs_ssl_match_cipher(left, tls13_cipher, tls13_offset, &support, CM_TRUE) != CM_SUCCESS) {
                return CM_ERROR;
            }
        }
...
static status_t cs_ssl_set_cipher(SSL_CTX *ctx, const ssl_config_t *config, bool32 *is_using_tls13)
{
...
static int32 ssl_get_expire_day(const ASN1_TIME *ctm, time_t *curr_time)
{
    int day, sec;
...
{
    SSL_CTX_free(SSL_CTX_PTR(ssl_ctx));
    cs_ssl_deinit();
}
...
static status_t cs_ssl_recv_remain(ssl_link_t *link, char *buf, uint32 offset, uint32 remain_size, uint32 wait_event,
    uint32 timeout)
{
    int32 recv_size;
    uint32 wait_interval = 0;
...
status_t cs_ssl_verify_certificate(ssl_link_t *link, ssl_verify_t vmode, const char *name, const char **errptr)
{
    ...
    switch (vmode) {
        ...
        case VERIFY_ISSUER:
            ...
            if ((attr != NULL) && (name != NULL) && cm_str_equal(name, get_common_name(attr, buf, sizeof(buf)))) {
                ret = CM_SUCCESS;
            } else {
