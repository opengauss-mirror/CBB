...
static void cs_reset_conn_timeout(socket_t sock)
{
...
static int32 cs_tcp_poll(struct pollfd *fds, uint32 nfds, int32 timeout)
{
#ifndef WIN32
...
static int32 cs_tcp_poll_check(const tcp_link_t *link, uint32 wait_for, time_t end_time)
{
    struct pollfd fd;
    int32 tv;
...
static status_t cs_tcp_connect_wait(const tcp_link_t *link, int32 error_no, time_t end_time)
{
    int32 ret = -1;
    int32 opt_val;
...
static status_t cs_tcp_connect_core(tcp_link_t *link, const socket_attr_t *sock_attr)
{
    int32 ret;
    int32 error_no;
    time_t end_time;
    ...
    if (ret < 0) {
        if (sock_attr->connect_timeout < 0) {
            ...
            end_time = cm_current_time() + ((uint32)sock_attr->connect_timeout) / MILLISECS_PER_SECOND;
        }
