...
#define TIMEVAL_DIFF_US(t_start, t_end) \
    (((t_end)->tv_sec - (t_start)->tv_sec) * 1000000ULL + (t_end)->tv_usec - (t_start)->tv_usec)
...
static inline status_t cm_date2str_ex(date_t date, const text_t *fmt_text, char *str, uint32 max_len)
{
    text_t date_text;
    date_text.str = str;
    date_text.len = 0;
