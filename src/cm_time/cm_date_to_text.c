...
{
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%04u", para->detail->year));
...
errno_t cm_fmt_hour_of_day12(const append_date_text_para_t *para, append_date_text_res_t *res)
{
    ...
    if (para->detail->hour == 0) {
        hh12_value = CM_HOURS_OF_DAY12;
    } else if (para->detail->hour > CM_HOURS_OF_DAY12) {
        hh12_value = para->detail->hour - CM_HOURS_OF_DAY12;
    } else {
        hh12_value = para->detail->hour;
    }
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02u", hh12_value));
    return CM_SUCCESS;
}
...
{
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02u", para->detail->hour));
    return CM_SUCCESS;
}
...
{
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02u", para->detail->sec));
    return CM_SUCCESS;
}

errno_t cm_fmt_second_pass(const append_date_text_para_t *para, append_date_text_res_t *res)
{
    PRTS_RETURN_IFERR(snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%05u",
        para->detail_ex->seconds));
    return CM_SUCCESS;
}
...
{
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02u", para->detail_ex->week));
    return CM_SUCCESS;
}
...
{
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02u", (para->detail->day)));
    return CM_SUCCESS;
}
...
errno_t cm_fmt_frac_second4_to_second6(const append_date_text_para_t *para, append_date_text_res_t *res)
{
    ...
    PRTS_RETURN_IFERR(snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%06u", frac));
...
{
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02u", para->detail->min));
    return CM_SUCCESS;
}
...
{
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02u", para->detail->mon));
    return CM_SUCCESS;
}
...
errno_t cm_fmt_tz_hour(const append_date_text_para_t *para, append_date_text_res_t *res)
{
    ...
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%+03d", tz_hour));
    return CM_SUCCESS;
}
...
errno_t cm_fmt_tz_minute(const append_date_text_para_t *para, append_date_text_res_t *res)
{
    ...
    PRTS_RETURN_IFERR(
        snprintf_s(res->item_str, FORMAT_ITEM_BUFFER_SIZE, FORMAT_ITEM_BUFFER_SIZE - 1, "%02d", tz_minute));
    return CM_SUCCESS;
}
