...
static inline int32 cm_get_day_of_week(const date_detail_t *detail)
{
    ...
    CM_ASSERT(day_of_week >= 0 && day_of_week < DAYS_PER_WEEK);
    return day_of_week;
}

static void cm_get_detail_ex(const date_detail_t *detail, date_detail_ex_t *detail_ex)
{
...
status_t cm_detail2text(const date_detail_t *detail, text_t *fmt, uint32 precision, text_t *text, uint32 max_len)
{
    ...
    while (fmt->len > 0) {
        ...
        CM_RETURN_IFERR(cm_append_date_text(detail, &detail_ex, item, &fmt_extra, precision, text, max_len));
    }
