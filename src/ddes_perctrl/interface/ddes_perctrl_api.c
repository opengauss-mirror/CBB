...
status_t perctrl_receive(int32 fd, perctrl_packet_t *msg)
{
    ...
    size = ((perctrl_cmd_head_t *)msg->buf)->size;
...
int32 perctrl_unregister_impl(const char *iof_dev, int64 rk, perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    ddes_init_get(ack);
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_RUN_ERR("[PERCTRL]unrgister failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
    ...
    perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_RUN_ERR("[PERCTRL]reverse failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
    ...
    perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_RUN_ERR("[PERCTRL]release failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
...
status_t perctrl_clear_impl(const char *iof_dev, int64 rk, perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_DEBUG_ERR("[PERCTRL]clear failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
    ...
    perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_RUN_ERR("[PERCTRL]preempt failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
    ...
    perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_DEBUG_ERR("[PERCTRL]caw failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
...
status_t perctrl_read_impl(ctrl_params_t *params, const char *iof_dev, perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_DEBUG_ERR("[PERCTRL]read failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
...
status_t perctrl_write_impl(ctrl_params_t *params, const char *iof_dev, perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_DEBUG_ERR("[PERCTRL]write failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
    ...
    perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_DEBUG_ERR("[PERCTRL]inql failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
    ...
    errno_t errcode = memcpy_sp((void *)inquiry_data, sizeof(inquiry_data_t), extra_info.str, sizeof(inquiry_data_t));
    ...
    perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_DEBUG_ERR("[PERCTRL]rkeys failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
    ...
    perctrl_packet_t *ack)
{
    ...
    if (ack->head->result != CM_SUCCESS) {
        (void)perctrl_handle_failed_ack(ack);
        LOG_DEBUG_ERR("[PERCTRL]rres failed, result:%d.", ack->head->result);
        return ack->head->result;
    }
