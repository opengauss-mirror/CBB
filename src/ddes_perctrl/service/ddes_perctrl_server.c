...
int32 exec_scsi3_reserve(perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    LOG_RUN_INF("Exec reserve ret %d, iof_dev: %s, rk: %lld, fd: %d, type: %u.", ret, iof_dev, rk, fd, type);
    (void)close(fd);
    return ret;
}
...
int32 exec_scsi3_release(perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    LOG_RUN_INF("Exec release ret %d, iof_dev: %s, rk: %lld, fd: %d, type: %u.", ret, iof_dev, rk, fd, type);
    (void)close(fd);
    return ret;
}
...
int32 exec_scsi3_preempt(perctrl_packet_t *req, perctrl_packet_t *ack)
{
    ...
    LOG_RUN_INF(
        "Exec preempt ret %d, iof_dev: %s, rk: %lld, sark: %lld, fd: %d, type: %u.", ret, iof_dev, rk, sark, fd, type);
    (void)close(fd);
    return ret;
}
